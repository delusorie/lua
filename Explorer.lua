getgenv().BriExecutedCheck = false
...
do
    ...
    for _, Host in Hosts do
        for _, Child in Host:GetChildren() do
            if Child.Name == "BriExplorer" then
                pcall(function()
                    Child:Destroy()
                end)
            end
...
local ErrorLogFolder = "Bri/ErrorLogs"
...
local function InitialiseErrorLog()
    ...
    pcall(function()
        local Header = `=== Bri Explorer error log session started {os.date("%Y-%m-%d %H:%M:%S")} ===\n`
...
local function Handle(Callback, FunctionName)
    local Failed, Info = xpcall(Callback, function(ErrorMessage)
        ...
        if KillScript or BriExecutedCheck then
            return task.wait(9e9)
        end

        local Formatted = `[Bri] {FunctionName} Error at line {LineNumber} [Time: {DateTime.now()}]: {ErrorMessage}`
...
local AssetFolder = "Bri"
local AssetsSubFolder = "Bri/Assets"
...
local function ApplyFPSCap(Unlimited)
    ...
    if Unlimited then
        ...
        if Good then
            BriAppliedUnlimited = true
        end

        return Good, Err
    else
        if BriAppliedUnlimited then
            local Good, Err = pcall(SetFn, UserOriginalFPSCap)
            if Good then
                BriAppliedUnlimited = false
            end
...
function BriUI:CreateInstance(ClassName, Properties)
...
function BriUI:AddStroke(Parent, ColorOrThemeKey, Thickness, Transparency)
...
function BriUI:AddPadding(Parent, Top, Right, Bottom, Left)
...
function BriUI:AddListLayout(Parent, Padding, Direction)
...
function BriUI:Tween(Object, Properties, TweenInfoOverride)
    Services.TweenService:Create(Object, TweenInfoOverride or TweenSnappy, Properties):Play()
end

function BriUI:CreateScreenGui()
...
function BriUI:CreateWindow(Config)
    ...
    if Brand then
        ...
        local BrandVersion = self:CreateInstance("TextLabel", {
            ...
            BackgroundTransparency = 1;
            Font = Fonts.Bold;
            Text = `BRI [{Explorer:FetchVersion()}]`;
            TextColor3 = Theme.Text;
            TextSize = 13;
            LayoutOrder = 1;
            ZIndex = 4;
    ...
    function Class:AddTitleButton(Text, Width, IsDanger, OnClick, IconAssetName, TextSizeOverride, IconSizeOverride)
        ...
        local Button = BriUI:CreateInstance("TextButton", {
        ...
        local Stroke = BriUI:AddStroke(Button, IsDanger and "Accent" or "Border", 1)

        local Icon
        if AssetId then
            Icon = BriUI:CreateInstance("ImageLabel", {
...
function BriUI:CreateClassIcon(ClassName, Parent)
...
function BriUI:CreateTooltip(Parent, Text)
...
function BriUI:ApplyClassIcon(ImageLabel, ClassName)
...
function Explorer:BeginRename(Node)
    ...
    local Box = BriUI:CreateInstance("TextBox", {
        Size = Label.Size;
    ...
    BriUI:AddStroke(Box, Theme.Accent, 1)
...
function Explorer:_FlatAllocateRow(Container)
    local Row = BriUI:CreateInstance("TextButton", {
    ...
    local Icon = BriUI:CreateInstance("ImageLabel", {
        Size = UDim2.new(0, 16, 0, 16);
    ...
    local Label = BriUI:CreateInstance("TextLabel", {
    ...
    local Path = BriUI:CreateInstance("TextLabel", {
...
function Explorer:_FlatBindRow(PoolEntry, Item, IndexY)
    ...
    if PoolEntry.IconClassName ~= ClassName then
        local NewIcon = BriUI:CreateClassIcon(ClassName, PoolEntry.Row)
...
function Explorer:_FlatEnsureContainer()
    ...
    local Container = BriUI:CreateInstance("Frame", {
...
function Explorer:_VTreeAllocateRow(Container)
    ...
    local AccentStrip = BriUI:CreateInstance("Frame", {
    ...
    local Arrow = BriUI:CreateInstance("TextLabel", {
    ...
    local ArrowHit = BriUI:CreateInstance("TextButton", {
...
function Explorer:_VTreeBindRow(PoolEntry, RowData, YPos)
    ...
    if RowData.IsTruncationNotice then
        ...
        BriUI:ApplyClassIcon(PoolEntry.Icon, nil)
    ...
    if PoolEntry.IconClassName ~= ClassName then
        BriUI:ApplyClassIcon(PoolEntry.Icon, ClassName)
        PoolEntry.IconClassName = ClassName
    end
...
function Explorer:CreatePropertyRow(GroupParent)
    return BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 0, 24);
        BackgroundColor3 = Theme.Field;
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
...
function Explorer:CreatePropertyLabel(Row, PropertyName)
    return BriUI:CreateInstance("TextLabel", {
...
function Explorer:AttachPropertyCopyHandler(Row, Object, PropertyName, DisplayName)
    ...
    local ClickCatcher = BriUI:CreateInstance("Frame", {
...
function Explorer:CreateTextRow(Object, PropertyName, Value, Editable, Parent)
    ...
    local Input = BriUI:CreateInstance(Editable and "TextBox" or "TextLabel", {
...
function Explorer:CreateBooleanRow(Object, PropertyName, Value, Parent)
    ...
    local Toggle = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(Toggle, "Border", 1)
...
function BriUI:BindThemeColor(Object, PropertyName, ThemeKey)
...
function Explorer:CreateColorRow(Object, PropertyName, Value, Parent)
    ...
    local TextLabel = BriUI:CreateInstance("TextLabel", {
    ...
    local Swatch = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(Swatch, "Border", 1)
...
function Explorer:CreateEnumRow(Object, PropertyName, Value, Parent)
    ...
    BriUI:AddStroke(Button, "Border", 1)
    BriUI:AddPadding(Button, 0, 8, 0, 8)
...
function Explorer:CreatePropertyCategoryHeader(Parent, CategoryName)
    local Holder = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 0, 22);
        BackgroundTransparency = 1;
        Parent = Parent;
    })

    BriUI:CreateInstance("TextLabel", {
        Size = UDim2.new(1, 0, 1, 0);
        BackgroundTransparency = 1;
        Font = Fonts.Bold;
...
function Explorer:RenderTagsSection(Object, Parent, OrderStart)
    ...
    for _, Tag in Tags do
        local Row = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 22);
            BackgroundTransparency = 1;
            LayoutOrder = Order;
            Parent = Parent;
        ...
        local RemoveButton = BriUI:CreateInstance("TextButton", {
        ...
        BriUI:AddStroke(RemoveButton, "Border", 1)
...
function Explorer:RenderProperties(Object)
    Handle(function()
        ...
        local function AddDivider()
            local Divider = BriUI:CreateInstance("Frame", {
        ...
        for _, Category in BucketOrder do
            ...
            local GroupHolder = BriUI:CreateInstance("Frame", {
                Size = UDim2.new(1, 0, 0, 0);
            ...
            BriUI:AddListLayout(GroupHolder, 1, Enum.FillDirection.Vertical)
        ...
        if #Attributes > 0 or Filter == "" then
            ...
            local AttributeHolder = BriUI:CreateInstance("Frame", {
                Size = UDim2.new(1, 0, 0, 0);
                AutomaticSize = Enum.AutomaticSize.Y;
                BackgroundTransparency = 1;
                Parent = self.PropertiesContent;
            })
            BriUI:AddListLayout(AttributeHolder, 1, Enum.FillDirection.Vertical)
            ...
            if #Attributes == 0 then
                ...
                for _, Entry in Attributes do
                    ...
                    if IsBooleanLikeAttribute then
                        ...
                        BriUI:AddStroke(Toggle, InitialBoolValue and Theme.Accent or "Border", 1)
            ...
            local AttributeButtonRow = BriUI:CreateInstance("Frame", {
                Size = UDim2.new(1, 0, 0, 26);
                BackgroundTransparency = 1;
                Parent = self.PropertiesContent;
            })

            local AttributeButtonLayout = BriUI:AddListLayout(
                AttributeButtonRow,
                4,
        ...
        local TagsHolder = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 0);
            AutomaticSize = Enum.AutomaticSize.Y;
            BackgroundTransparency = 1;
            Parent = self.PropertiesContent;
        })
        BriUI:AddListLayout(TagsHolder, 1, Enum.FillDirection.Vertical)
        ...
        local TagButtonRow = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 26);
            BackgroundTransparency = 1;
            Parent = self.PropertiesContent;
        })
...
function Explorer:CreateModalWindow(Title, Width, Height, Options)
    ...
    if not Options.Floating then
        Blocker = BriUI:CreateInstance("TextButton", {
    ...
    local Window = BriUI:CreateInstance("Frame", {
    ...
    local WindowStroke = BriUI:AddStroke(Window, "Border", 1)
    ...
    local TitleBar = BriUI:CreateInstance("Frame", {
    ...
    local ModalBodyBackground = BriUI:CreateInstance("Frame", {
    ...
    local CloseButton = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(CloseButton, "Border", 1)

    local CloseIcon
    if CloseAssetId then
        CloseIcon = BriUI:CreateInstance("ImageLabel", {
    ...
    local Body = BriUI:CreateInstance("Frame", {
    ...
    if Options.Resizable then
        ...
        local function MakeEdge(Position, Size, DirX, DirY)
            local Edge = BriUI:CreateInstance("Frame", {
                Position = Position;
                Size = Size;
                BackgroundTransparency = 1;
                ZIndex = 210;
...
function Explorer:OpenColorPicker(CurrentColor, Callback, Options)
    ...
    BriUI:AddListLayout(Body, 8, Enum.FillDirection.Vertical)
    ...
    local Preview = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddStroke(Preview, "Border", 1)

    local SVBox = BriUI:CreateInstance("ImageButton", {
    ...
    local WhiteOverlay = BriUI:CreateInstance("Frame", {
        Size = UDim2.fromScale(1, 1);
    ...
    BriUI:CreateInstance("UIGradient", {
    ...
    local BlackOverlay = BriUI:CreateInstance("Frame", {
        Size = UDim2.fromScale(1, 1);
    ...
    local SVCursor = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddStroke(SVCursor, Color3.new(1,1,1), 2)

    local HueStrip = BriUI:CreateInstance("ImageButton", {
        Size = UDim2.new(1, 0, 0, 18);
        BackgroundColor3 = Color3.new(1,1,1);
        BorderSizePixel = 0;
        AutoButtonColor = false;
    ...
    local HueCursor = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddStroke(HueCursor, Color3.fromRGB(0,0,0), 1)
    ...
    local function CreateChannelSlider(LabelText, ChannelColor, GetVal, SetVal, OrderIndex)
        ...
        local ValueLabel = BriUI:CreateInstance("TextLabel", {
        ...
        local Tk = BriUI:CreateInstance("Frame", {
        ...
        local Fill = BriUI:CreateInstance("Frame", {
        ...
        local Hit = BriUI:CreateInstance("TextButton", {
            Size = UDim2.new(1, 0, 0, 22);
...
function Explorer:OpenListModal(Title, Items, ItemTextFunction, OnPick, ShowSearch, ItemHeight, ItemIconFunction, Options)
    ...
    local Layout = BriUI:AddListLayout(Body, 6, Enum.FillDirection.Vertical)
    local Filter = ""
    local SearchBox

    if ShowSearch then
        SearchBox = BriUI:CreateInstance("TextBox", {
            Size = UDim2.new(1, 0, 0, 26);
            BackgroundColor3 = Theme.Field;
            BorderSizePixel = 0;
            Font = Fonts.Mono;
        ...
        BriUI:AddStroke(SearchBox, "Border", 1)
        BriUI:AddPadding(SearchBox, 0, 8, 0, 8)
    ...
    local Scroll = BriUI:CreateInstance("ScrollingFrame", {
    ...
    BriUI:BindThemeColor(Scroll, "ScrollBarImageColor3", "Border")

    local ListLayout = BriUI:AddListLayout(Scroll, 2, Enum.FillDirection.Vertical)
    ...
    local function Render()
        ...
        for _, Item in Items do
            ...
            if LoweredFilter == "" or Text:lower():find(LoweredFilter, 1, true) then
                ...
                BriUI:AddPadding(Button, Multiline and 4 or 0, 8, Multiline and 4 or 0, LeftPad)

                if IconAssetName then
                    local IconAsset = GetClassAssetId(IconAssetName)
                    if IconAsset then
                        BriUI:CreateInstance("ImageLabel", {
                            Size = UDim2.new(0, 14, 0, 14);
                ...
                Button.MouseEnter:Connect(function()
                    BriUI:Tween(Button, {BackgroundTransparency = 0})
                end)

                Button.MouseLeave:Connect(function()
                    BriUI:Tween(Button, {BackgroundTransparency = 0.5})
                end)
...
function Explorer:OpenAttributeModal(Title, ExistingName, ExistingValue)
    ...
    local NameBox = BriUI:CreateInstance("TextBox", {
    ...
    BriUI:AddStroke(NameBox, "Border", 1)
    BriUI:AddPadding(NameBox, 0, 6, 0, 6)
    ...
    local TypeBtn = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(1, -56, 1, 0);
        Position = UDim2.new(0, 56, 0, 0);
        BackgroundColor3 = Theme.Field;
        BorderSizePixel = 0;
    ...
    BriUI:AddStroke(TypeBtn, "Border", 1)
    BriUI:AddPadding(TypeBtn, 0, 8, 0, 8)

    local DropdownLayer = BriUI:CreateInstance("Frame", {
        Size = UDim2.fromScale(1, 1);
    ...
    local TypeDropdown = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddStroke(TypeDropdown, "Border", 1)

    BriUI:CreateInstance("UIListLayout", {
    ...
    for Index, Type in AttributeTypes do
        local Item = BriUI:CreateInstance("TextButton", {
            Size = UDim2.new(1, 0, 0, 22);
            BackgroundColor3 = Theme.Field;
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
    ...
    local ValueBox = BriUI:CreateInstance("TextBox", {
        Size = UDim2.new(1, -56, 1, 0);
        Position = UDim2.new(0, 56, 0, 0);
        BackgroundColor3 = Theme.Field;
        BorderSizePixel = 0;
    ...
    BriUI:AddStroke(ValueBox, "Border", 1)
    BriUI:AddPadding(ValueBox, 0, 6, 0, 6)

    local BtnRow = BriUI:CreateInstance("Frame", {
    ...
    local BL = BriUI:AddListLayout(BtnRow, 8, Enum.FillDirection.Horizontal)
    BL.HorizontalAlignment = Enum.HorizontalAlignment.Right

    local function MakeBtn(Text, Accent)
        local B = BriUI:CreateInstance("TextButton", {
        ...
        BriUI:AddStroke(B, Accent and Theme.Accent or "Border", 1)
        return B
    end
...
function Explorer:OpenAddTagModal()
    ...
    BriUI:AddStroke(Box, "Border", 1)
    BriUI:AddPadding(Box, 0, 8, 0, 8)
    ...
    local L = BriUI:AddListLayout(BtnRow, 8, Enum.FillDirection.Horizontal)
...
function Explorer:Open3DPreview(Instance)
    ...
    local PreviewWindow = BriUI:CreateWindow({
        Parent = self.ScreenGui;
    ...
    local ViewportContainer = BriUI:CreateInstance("Frame", {
    ...
    local ViewportFrame = BriUI:CreateInstance("ViewportFrame", {
        Size = UDim2.fromScale(1, 1);
        BackgroundColor3 = Color3.fromRGB(0, 0, 0);
...
function Explorer:OpenMethodCaller(Method)
    ...
    BriUI:AddListLayout(Body, 6, Enum.FillDirection.Vertical)
    ...
    local FieldsScroll = BriUI:CreateInstance("ScrollingFrame", {
    ...
    if UseGrid then
        BriUI:CreateInstance("UIGridLayout", {
    ...
    local ResultScroll = BriUI:CreateInstance("ScrollingFrame", {
    ...
    BriUI:AddPadding(ResultScroll, 6, 8, 6, 8)

    local ResultLabel = BriUI:CreateInstance("TextLabel", {
    ...
    BriUI:AddPadding(ResultLabel, 6, 8, 6, 8)

    local CallButton = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(1, 0, 0, 28);
        BackgroundColor3 = Theme.Accent;
    ...
    BriUI:AddStroke(CallButton, Theme.Accent, 1)
    ...
    local CopyOutputButton = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(1, 0, 0, 24);
        BackgroundColor3 = Theme.Field;
    ...
    BriUI:AddStroke(CopyOutputButton, "Border", 1)
...
function Explorer:ShowConnectionsResult(Connections)
    ...
    BriUI:AddListLayout(Scroll, 4, Enum.FillDirection.Vertical)
    ...
    for Index, Conn in Connections do
        ...
        BriUI:AddStroke(Row, "Border", 1)
        BriUI:AddPadding(Row, 6, 8, 6, 8)
        ...
        local Header = BriUI:CreateInstance("TextLabel", {
            Size = UDim2.new(1, 0, 0, 14);
            BackgroundTransparency = 1;
            Font = Fonts.Mono;
        ...
        local FnLabel = BriUI:CreateInstance("TextLabel", {
            Size = UDim2.new(1, 0, 0, 14);
            Position = UDim2.new(0, 0, 0, 16);
            BackgroundTransparency = 1;
            Font = Fonts.Mono;
        ...
        local BtnStrip = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 18);
        ...
        local function MakeBtn(LabelText, Width, OnClick)
            BtnOrder += 1
            local Btn = BriUI:CreateInstance("TextButton", {
            ...
            BriUI:AddStroke(Btn, "Border", 1)
            Btn.MouseButton1Click:Connect(OnClick)
            Btn.MouseEnter:Connect(function()
                BriUI:Tween(Btn, {BackgroundTransparency = 0.2})
            end)
            Btn.MouseLeave:Connect(function()
                BriUI:Tween(Btn, {BackgroundTransparency = 0})
            end)
            return Btn
        end
...
function Explorer:OpenCallRemote()
    ...
    local FireButton = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(1, 0, 0, 28);
        BackgroundColor3 = Theme.Accent;
        BackgroundTransparency = 0.85;
        BorderSizePixel = 0;
    ...
    BriUI:AddStroke(FireButton, Theme.Accent, 1)
...
function Explorer:OpenConstantUpvalueSearch(ScriptObject)
    ...
    local TopBar = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 0, 26);
        Position = UDim2.new(0, 0, 0, 0);
        BackgroundTransparency = 1;
        ZIndex = 202;
    ...
    local SearchBox = BriUI:CreateInstance("TextBox", {
    ...
    local CountLabel = BriUI:CreateInstance("TextLabel", {
    ...
    local Results = BriUI:CreateInstance("ScrollingFrame", {
    ...
    BriUI:AddStroke(Results, "BorderSoft", 1)
    ...
    BriUI:AddPadding(Results, 4, 6, 4, 6)
    ...
    local function AddResultRow(Kind, FnIndex, FnLabel, EntryIdx, Value)
        ...
        BriUI:AddStroke(Row, "BorderSoft", 1)
        ...
        local Catcher = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 1, 0);
            BackgroundTransparency = 1;
            ZIndex = 205;
            Parent = Row;
...
function Explorer:OpenScriptViewer(ScriptObject, UseDefault)
    ...
    BriUI:CreateInstance("TextButton", {
        Size = UDim2.fromScale(1, 1);
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
        AutoButtonColor = false;
    ...
    local ButtonStrip = BriUI:CreateInstance("Frame", {
    ...
    DumpScriptButton = MakeStripButton("Dump Script", 86, function()
        local Path = `bri_dumps/{SafeFileBase()}.luau`
        local FileOK = false
        if writefile and isfolder and makefolder then
            FileOK = pcall(function()
                if not isfolder("bri_dumps") then makefolder("bri_dumps") end
    ...
    DumpFunctionsButton = MakeStripButton("Dump Functions", 108, function()
        ...
        local Path = `bri_dumps/{SafeFileBase()}.functions.txt`
        local FileOK = false
        if writefile and isfolder and makefolder then
            FileOK = pcall(function()
                if not isfolder("bri_dumps") then makefolder("bri_dumps") end
    ...
    DumpBytecodeButton = MakeStripButton("Dump Bytecode", 102, function()
        ...
        local Path = `bri_dumps/{SafeFileBase()}.bytecode.bin`
        local FileOK = false
        if writefile and isfolder and makefolder then
            FileOK = pcall(function()
                if not isfolder("bri_dumps") then makefolder("bri_dumps") end
    ...
    local CloseStroke = BriUI:AddStroke(CloseButton, Theme.Accent, 1)
    ...
    BriUI:AddStroke(Body, Theme.BorderSoft, 1)
    ...
    local GutterHolder = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddPadding(GutterHolder, 6, 8, 6, 0)

    local CodeHolder = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddPadding(CodeHolder, 6, 8, 6, 4)
...
function Explorer:OpenConsole()
    ...
    local ConsoleWindow = BriUI:CreateWindow({
        Parent = self.ScreenGui;
        Title = "Console";
    ...
    local TopBarLayout = BriUI:AddListLayout(TopBar, 6, Enum.FillDirection.Horizontal)
    ...
    local SizeLabel = BriUI:CreateInstance("TextLabel", {
    ...
    local LogScrollFrame = BriUI:CreateInstance("ScrollingFrame", {
    ...
    BriUI:AddPadding(LogScrollFrame, 4, 6, 4, 6)
    BriUI:AddListLayout(LogScrollFrame, 1, Enum.FillDirection.Vertical)
    ...
    local CommandRow = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, -16, 0, 28);
        Position = UDim2.new(0, 8, 1, -36);
        BackgroundTransparency = 1;
        Parent = WindowBody;
    })
    local CommandLayout = BriUI:AddListLayout(CommandRow, 6, Enum.FillDirection.Horizontal)
    CommandLayout.VerticalAlignment = Enum.VerticalAlignment.Center

    local CommandBox = BriUI:CreateInstance("TextBox", {
    ...
    BriUI:AddStroke(CommandBox, "Border", 1)
    BriUI:AddPadding(CommandBox, 0, 8, 0, 8)

    local RunButton = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(RunButton, Theme.Accent, 1)
    ...
    local function ExecuteCommand()
        ...
        local function CapturedLoadstring(Source, ChunkName)
            ...
            local Fn, Err = OriginalLoadstring(Injected, ChunkName or "BriConsoleChunk")
        ...
        local Fn, Err = CapturedLoadstring(Body, "BriConsole")
        if not Fn then
...
function Explorer:OpenSettings()
    ...
    BriUI:BindThemeColor(Scroll, "ScrollBarImageColor3", "Border")
    BriUI:AddPadding(Scroll, 8, 16, 8, 12)
    BriUI:AddListLayout(Scroll, 6, Enum.FillDirection.Vertical)
    ...
    local function CreateToggle(LabelText, InitialState, OrderIndex, OnChange)
        ...
        local Switch = BriUI:CreateInstance("TextButton", {
        ...
        BriUI:AddStroke(Switch, "Border", 1)

        local Knob = BriUI:CreateInstance("Frame", {
        ...
        Switch.MouseButton1Click:Connect(function()
            State = not State
            BriUI:Tween(Switch, {BackgroundColor3 = State and Theme.Accent or Theme.ToggleOff})
            BriUI:Tween(Knob, {Position = State and UDim2.new(1, -14, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)})
            OnChange(State)
        end)
    end
    ...
    local function CreateSlider(LabelText, MinValue, MaxValue, Step, InitialValue, OrderIndex, OnChange, FormatValueText)
        ...
        local TrackFrame = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 6);
        ...
        local HitArea = BriUI:CreateInstance("TextButton", {
            Size = UDim2.new(1, 0, 1, 0);
            BackgroundTransparency = 1;
            AutoButtonColor = false;
            Text = "";
    ...
    local KeyButton = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(KeyButton, "Border", 1)
    ...
    local PresetButton = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(PresetButton, "Border", 1)
    BriUI:AddPadding(PresetButton, 0, 8, 0, 8)
    ...
    local ResetColorsButton = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(1, 0, 0, 24);
        BackgroundColor3 = Theme.Field;
        BorderSizePixel = 0;
        AutoButtonColor = false;
    ...
    BriUI:AddStroke(ResetColorsButton, "Border", 1)

    ResetColorsButton.MouseEnter:Connect(function()
        BriUI:Tween(ResetColorsButton, {TextColor3 = Theme.Text})
    end)
    ResetColorsButton.MouseLeave:Connect(function()
        BriUI:Tween(ResetColorsButton, {TextColor3 = Theme.TextDim})
    end)
    ...
    local ResetButton = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(1, 0, 0, 24);
        BackgroundColor3 = Theme.Field;
        BorderSizePixel = 0;
        AutoButtonColor = false;
    ...
    BriUI:AddStroke(ResetButton, "Border", 1)

    ResetButton.MouseEnter:Connect(function()
        BriUI:Tween(ResetButton, {TextColor3 = Theme.Text})
    end)
    ResetButton.MouseLeave:Connect(function()
        BriUI:Tween(ResetButton, {TextColor3 = Theme.TextDim})
    end)
...
function Explorer:OpenFiltersDropdown(AnchorButton)
    ...
    local Blocker = BriUI:CreateInstance("Frame", {
        Size = UDim2.fromScale(1, 1);
        BackgroundTransparency = 1;
        ZIndex = 150;
        Parent = self.ScreenGui;
    ...
    local Window = BriUI:CreateWindow({
        Parent = self.ScreenGui;
        Title = "Filters";
        BackgroundTransparency = 1;
        Size = UDim2.fromOffset(Width, Height);
    ...
    BriUI:AddListLayout(Scroll, 8, Enum.FillDirection.Vertical)
    BriUI:AddPadding(Scroll, 0, 8, 0, 0)

    self._FilterRowRefreshers = {}

    local function CreateSection(Title, OrderIndex, BuildContent)
        local Section = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 0);
            AutomaticSize = Enum.AutomaticSize.Y;
            BackgroundTransparency = 1;
            LayoutOrder = OrderIndex;
        ...
        BriUI:AddListLayout(Section, 6, Enum.FillDirection.Vertical)
        BriUI:CreateInstance("TextLabel", {
            Size = UDim2.new(1, 0, 0, 16);
            BackgroundTransparency = 1;
            Font = Fonts.Bold;
        ...
        local Content = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 0);
            AutomaticSize = Enum.AutomaticSize.Y;
            BackgroundTransparency = 1;
            LayoutOrder = 2;
    ...
    local function CreateToggleRow(Parent, LabelText, FullWidth, OrderIndex, IconClassName, IsOn, OnClick)
        ...
        BriUI:AddPadding(Row, 0, 6, 0, 6)

        local Check = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(0, 12, 0, 12);
        ...
        if IconClassName then
            local Icon = BriUI:CreateClassIcon(IconClassName, Row)
        ...
        Row.MouseEnter:Connect(function()
            BriUI:Tween(Row, {BackgroundTransparency = 0.4})
        end)

        Row.MouseLeave:Connect(function()
            BriUI:Tween(Row, {BackgroundTransparency = 0.7})
        end)
    ...
    CreateSection("Nil Instances", 1, function(Container)
        local Inner = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 0);
            AutomaticSize = Enum.AutomaticSize.Y;
            BackgroundTransparency = 1;
            ZIndex = 152;
        ...
        BriUI:AddListLayout(Inner, 4, Enum.FillDirection.Vertical)
        ...
        local ClassBox = BriUI:CreateInstance("TextBox", {
            Size = UDim2.new(1, 0, 0, 22);
            BackgroundColor3 = Theme.Field;
            BorderSizePixel = 0;
            Font = Fonts.Mono;
        ...
        BriUI:AddStroke(ClassBox, "Border", 1)
        BriUI:AddPadding(ClassBox, 0, 6, 0, 6)
    ...
    CreateSection("Freeze Search", 2, function(Container)
        ...
        local Hint = BriUI:CreateInstance("TextLabel", {
            Size = UDim2.new(1, 0, 0, 28);
            BackgroundTransparency = 1;
            Font = Fonts.Medium;
        ...
        local Status = BriUI:CreateInstance("TextLabel", {
    ...
    CreateSection("Search - ClassName Filter", 4, function(Container)
        local Grid = BriUI:CreateInstance("Frame", {
            Size = UDim2.new(1, 0, 0, 0);
            AutomaticSize = Enum.AutomaticSize.Y;
            BackgroundTransparency = 1;
            ZIndex = 152;
...
function Explorer:CreateSearchBar()
    ...
    BriUI:AddStroke(SearchBox, "Border", 1)
    BriUI:AddPadding(SearchBox, 0, RightPad, 0, LeftPad)
    BindTheme("Field", function(Color)
        SearchBox.BackgroundColor3 = Color
    end)
    ...
    if SearchIconAsset then
        local SearchIcon = BriUI:CreateInstance("ImageLabel", {
            Size = UDim2.new(0, 14, 0, 14);
    ...
    local FiltersButton = BriUI:CreateInstance("TextButton", {
        Size = UDim2.new(0, 56, 0, 18);
    ...
    BriUI:AddStroke(FiltersButton, "Border", 1)
    BindTheme("Border", function(Color)
    ...
    FiltersButton.MouseEnter:Connect(function()
        BriUI:Tween(FiltersButton, {BackgroundColor3 = Theme.Selected; TextColor3 = Theme.Text})
    end)

    FiltersButton.MouseLeave:Connect(function()
        BriUI:Tween(FiltersButton, {BackgroundColor3 = Theme.Border; TextColor3 = Theme.TextDim})
    end)
...
function Explorer:CreatePropertyFilterBar()
    ...
    local FilterBox = BriUI:CreateInstance("TextBox", {
        Size = UDim2.new(1, -16, 0, 24);
        Position = UDim2.new(0, 8, 0.5, -12);
        BackgroundColor3 = Theme.Field;
        BorderSizePixel = 0;
    ...
    BriUI:AddStroke(FilterBox, "Border", 1)
    BriUI:AddPadding(FilterBox, 0, 8, 0, 8)
...
function Explorer:BuildQuickAccessBar()
    ...
    local QuickButton = BriUI:CreateInstance("TextButton", {
    ...
    BriUI:AddStroke(QuickButton, "Border", 1)
    ...
    local function MakeTile(GridParent, LabelText, GetActive, Order, Callback)
        ...
        local Tile = BriUI:CreateInstance("TextButton", {
        ...
        local Stroke = BriUI:AddStroke(Tile, IsActive and "SelectionBar" or "Border", 1)

        local InnerFrame = BriUI:CreateInstance("Frame", {
            Size = UDim2.fromScale(1, 1);
            BackgroundTransparency = 1;
            ZIndex = 302;
            Parent = Tile;
        })
        local InnerLayout = BriUI:AddListLayout(InnerFrame, 4, Enum.FillDirection.Vertical)
    ...
    local function OpenDropdown()
        ...
        local Panel = BriUI:CreateInstance("Frame", {
        ...
        local PanelStroke = BriUI:AddStroke(Panel, "Border", 1)
        BriUI:AddPadding(Panel, 4, 10, 12, 10)
        BriUI:AddListLayout(Panel, 8, Enum.FillDirection.Vertical)
...
function Explorer:_StartClickUiToSelect()
    ...
    self._ClickUiConnection = Track(Mouse.Button1Down:Connect(function()
        ...
        if self:IsPointOverBriUi(Mouse.X, Mouse.Y) then
            return
        end
...
function Explorer:IsPointOverBriUi(X, Y)
...
function Explorer:_RebuildPinBar()
    ...
    for I, Path in self.PinnedPaths do
        ...
        local Pill = BriUI:CreateInstance("TextButton", {
            Size = UDim2.new(0, 0, 1, 0);
            AutomaticSize = Enum.AutomaticSize.X;
            BackgroundColor3 = Theme.Field;
            BorderSizePixel = 0;
        ...
        BriUI:AddStroke(Pill, "Border", 1)
        BriUI:AddPadding(Pill, 0, 6, 0, 6)
        ...
        BriUI:ApplyClassIcon(Icon, ClassName)
...
function Explorer:BuildExplorerWindow()
    ...
    local ActionStrip = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, -16, 0, 26);
        Position = UDim2.new(0, 8, 0, 6);
        BackgroundTransparency = 1;
        Parent = Window.Body;
    ...
    local ActionLayout = BriUI:AddListLayout(ActionStrip, 6, Enum.FillDirection.Horizontal)
    ...
    local function MakeAction(LabelText, OrderIndex, Callback)
        ...
        BriUI:AddStroke(Button, "Border", 1)
        BriUI:AddPadding(Button, 0, 10, 0, 10)
    ...
    MakeAction("Kick Self", 3, function()
        pcall(function()
            self.LocalPlayer:Kick("Kicked via BRI Explorer")
        end)
    end)

    local PinBar = BriUI:CreateInstance("ScrollingFrame", {
    ...
    local PinLayout = BriUI:AddListLayout(PinBar, 4, Enum.FillDirection.Horizontal)
    PinLayout.VerticalAlignment = Enum.VerticalAlignment.Center

    self.PinBar = PinBar

    local SearchHolder = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 0, 36);
    ...
    local TreeHolder = BriUI:CreateInstance("Frame", {
    ...
    local Tree = BriUI:CreateInstance("ScrollingFrame", {
        Size = UDim2.new(1, 0, 1, 0);
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
        ScrollBarThickness = 6;
    ...
    BriUI:BindThemeColor(Tree, "ScrollBarImageColor3", "Border")
    BriUI:AddPadding(Tree, 4, 8, 8, 8)
    BriUI:AddListLayout(Tree, 1, Enum.FillDirection.Vertical)

    local StickyHeader = BriUI:CreateInstance("TextButton", {
    ...
    local StickyDivider = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 0, 1);
    ...
    local StickyIcon = BriUI:CreateInstance("ImageLabel", {
        Size = UDim2.new(0, 16, 0, 16);
    ...
    local StickyLabel = BriUI:CreateInstance("TextLabel", {
        Size = UDim2.new(1, -40, 1, 0);
    ...
    local function UpdateStickyHeader()
        ...
        BriUI:ApplyClassIcon(StickyIcon, Anchor.ClassName)
    ...
    self.ExplorerColumn = {
        ...
        AddLabel = function(_, Text)
            ...
            BriUI:BindThemeColor(Label, "TextColor3", "TextDim")

            return Label
        end;
    }

    self.ReparentIndicator = BriUI:CreateInstance("TextLabel", {
        Size = UDim2.new(1, -16, 0, 22);
    ...
    BriUI:BindThemeColor(self.ReparentIndicator, "BackgroundColor3", "Accent")
...
function Explorer:SetupDragHandlers()
    ...
    Track(Services.UserInputService.InputChanged:Connect(function(Input)
        ...
        if not DragOperation.HasStarted and (DeltaX * DeltaX + DeltaY * DeltaY) > 64 then
            ...
            DragOperation.GhostLabel = BriUI:CreateInstance("TextLabel", {
            ...
            BriUI:AddStroke(DragOperation.GhostLabel, Theme.Accent, 1)
        end
...
function Explorer:BuildPropertiesWindow()
    ...
    local TitleStrip = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, -16, 0, 22);
        Position = UDim2.new(0, 8, 0, 8);
        BackgroundTransparency = 1;
        Parent = Window.Body;
    ...
    self.PropertiesTitleLabel = BriUI:CreateInstance("TextLabel", {
        Size = UDim2.new(1, 0, 1, 0);
        BackgroundTransparency = 1;
        Font = Fonts.Mono;
        Text = "(no selection)";
    ...
    local FilterHolder = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, -16, 0, 28);
    ...
    local ContentHolder = BriUI:CreateInstance("Frame", {
    ...
    local Content = BriUI:CreateInstance("ScrollingFrame", {
        Size = UDim2.new(1, 0, 1, 0);
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
        ScrollBarThickness = 3;
    ...
    BriUI:BindThemeColor(Content, "ScrollBarImageColor3", "Border")
    BriUI:AddPadding(Content, 4, 8, 8, 8)
    BriUI:AddListLayout(Content, 0, Enum.FillDirection.Vertical)
...
function Explorer:OpenDiffViewer(Inst)
    ...
    local List = BriUI:CreateInstance("ScrollingFrame", {
    ...
    BriUI:AddListLayout(List, 2, Enum.FillDirection.Vertical)
    BriUI:AddPadding(List, 4, 4, 4, 4)
    ...
    for _, Change in Changes do
        ...
        BriUI:AddPadding(Row, 4, 8, 4, 8)

        local TypeTag = BriUI:CreateInstance("TextLabel", {
...
function Explorer:OpenContextMenu(AnchorX, AnchorY)
    ...
    Menu = BriUI:CreateInstance("Frame", {
    ...
    BriUI:AddStroke(Menu, "Border", 1)
    BriUI:AddPadding(Menu, 6, 6, 6, 6)
    BriUI:AddListLayout(Menu, 2, Enum.FillDirection.Vertical)
    BindTheme("Window", function(Color)
    ...
    local function MakeItem(LabelText, Disabled, Callback)
        ...
        BriUI:AddPadding(Item, 0, 10, 0, 10)
        if Disabled then
            return Item
        end
        ...
        Track(Item.MouseLeave:Connect(function()
            BriUI:Tween(Item, {BackgroundTransparency = 1})
        end))
...
function Explorer:ShowNotification(Title, Message, Variant)
    ...
    if not self.NotificationHolder then
        self.NotificationHolder = BriUI:CreateInstance("Frame", {
            Name = "Notifications";
        ...
        local Layout = BriUI:AddListLayout(self.NotificationHolder, 6, Enum.FillDirection.Vertical)
        Layout.HorizontalAlignment = Enum.HorizontalAlignment.Right
        Layout.VerticalAlignment = Enum.VerticalAlignment.Bottom
    end

    local Card = BriUI:CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 0, 0);
        AutomaticSize = Enum.AutomaticSize.Y;
        BackgroundColor3 = Theme.Window;
        BackgroundTransparency = 1;
    ...
    local CardStroke = BriUI:AddStroke(Card, AccentColor, 1)
    CardStroke.Transparency = 1
    BriUI:AddPadding(Card, 9, 12, 9, 12)
    BriUI:AddListLayout(Card, 3, Enum.FillDirection.Vertical)

    local TitleLabel = BriUI:CreateInstance("TextLabel", {
        Size = UDim2.new(1, 0, 0, 14);
        BackgroundTransparency = 1;
        Font = Fonts.Bold;
        Text = (Title or "BRI"):upper();
    ...
    local Detail = BriUI:CreateInstance("TextLabel", {
        Size = UDim2.new(1, 0, 0, 0);
        AutomaticSize = Enum.AutomaticSize.Y;
        BackgroundTransparency = 1;
        Font = Fonts.Medium;
    ...
    BriUI:Tween(Card, {BackgroundTransparency = 0.05})
    BriUI:Tween(CardStroke, {Transparency = 0})
    BriUI:Tween(TitleLabel, {TextTransparency = 0})
    BriUI:Tween(Detail, {TextTransparency = 0})
    ...
    task.delay(Lifetime, function()
        ...
        BriUI:Tween(Card, {BackgroundTransparency = 1}, FadeInfo)
        BriUI:Tween(CardStroke, {Transparency = 1}, FadeInfo)
        BriUI:Tween(TitleLabel, {TextTransparency = 1}, FadeInfo)
        BriUI:Tween(Detail, {TextTransparency = 1}, FadeInfo)
...
function Explorer:ShowErrorNotification(Message)
    self:ShowNotification("BRI ERROR", Message or "Unknown error", "error")
end

function Explorer:Notify(Message)
    self:ShowNotification("BRI", Message, "success")
    --print(`BRI {Message} success`)
end
        ...
        if not isfile(self.ConfigPath) then
            ...
            if Data and Data.Version ~= self.Version then
                ...
                task.delay(3, function()
                    self:ShowNotification(`NEW UPDATE!`, `BRI has updated to {Data.Version}`, "info")
                end)
...
function ExplorerClass:Create()
    Handle(function()
        self:InitConfig()
        self.ScreenGui = BriUI:CreateScreenGui()
...
Explorer:SpawnTask("ScriptExecutedAgainCheck", function()
    ...
    while not KillScript do
        ...
        if BriExecutedCheck == false then
            Explorer:Kill()
